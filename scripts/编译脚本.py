...
class ReverseTranslator:
    ...
    def translate_directory(
        ...
        # 获取所有需要处理的文件（包括需要翻译的和需要复制的）
        files_to_translate, files_to_copy = self._collect_files(input_path, exclude_dirs, exclude_files)

        if not files_to_translate and not files_to_copy:
            print(f"警告: 在 {input_dir} 中未找到任何文件")
            return

        print(f"\n找到 {len(files_to_translate)} 个待反向翻译文件, {len(files_to_copy)} 个待复制文件")
        print("=" * 60)
        ...
        for file_path in files_to_copy:
            rel_path = file_path.relative_to(input_path)
            self._copy_file(file_path, output_path / rel_path)
    ...
    def _copy_file(self, input_path: Path, output_path: Path) -> bool:
        ...
        try:
            ...
            print(f"  [COPY] 已复制: {input_path}")
            return True
        except Exception as e:
            print(f"  [FAIL] 复制失败: {input_path} - {e}")
            return False
    ...
    def _collect_files(
        ...
        return sorted(files_to_translate), sorted(files_to_copy)
...
def main() -> None:
    ...
    # 默认输出目录修改为指定路径
    default_output_dir = r"D:\Users\Documents\MuMu共享文件夹\X\rustedWarfare\units\人机的玩笑"
